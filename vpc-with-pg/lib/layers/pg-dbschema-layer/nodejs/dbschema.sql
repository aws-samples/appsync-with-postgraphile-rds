...
comment on column forum_example.post.author_username is "The id of the author user.";
...
begin
  insert into forum_example.person (first_name, last_name, username) values
...
comment on function forum_example.register_person(text, text, text) is "Registers a single user and creates an account in our forum.";

create role appsync_graphile login password '<password>';

create role forum_example_anonymous;
grant forum_example_anonymous to appsync_graphile;

create role forum_example_person;
grant forum_example_person to appsync_graphile;

create function forum_example.current_person() returns forum_example.person as $$
  select *
  from forum_example.person
  where username = current_setting('identity.username')::text
$$ language sql stable;
...
grant execute on function forum_example.register_person(text, text, text) to forum_example_anonymous;
...
create policy update_person on forum_example.person for update to forum_example_person
  using (username = current_setting('identity.username')::text);
...
create policy insert_post on forum_example.post for insert to forum_example_person
  with check (author_username = current_setting('identity.username')::text);

create policy update_post on forum_example.post for update to forum_example_person
  using (author_username = current_setting('identity.username')::text);
